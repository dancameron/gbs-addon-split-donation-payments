...
class Group_Buying_Simple_Charities extends Group_Buying_Controller {
...
// Initiate the add-on
class Group_Buying_Simple_Charities_Addon extends Group_Buying_Controller {
	
	public static function init() {
		// Hook this plugin into the GBS add-ons controller
		add_filter('gb_addons', array(get_class(),'gb_simple_charities'), 10, 1);
	}

	public static function gb_simple_charities( $addons ) {
		$addons['gb_simple_charities'] = array(
			'label' => self::__('Simple Charities'),
			'description' => self::__('Adds a charity selection at checkout that doesn’t require deal attributes. New reports are also available.'),
			'files' => array(
				__FILE__,
				dirname( __FILE__ ) . '/library/template-tags.php',
			),
			'callbacks' => array(
				array('Group_Buying_Simple_Charities', 'init'),
			),
		);
		return $addons;
	}
